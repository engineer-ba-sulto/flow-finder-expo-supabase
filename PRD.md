...
1. **ゴール設定**：複数目標の登録・編集・優先度設定
2. **ユーザー認証**：Supabase Auth による認証機能
3. **基本 UI**：Button, Input, Card コンポーネント
4. **基本ナビゲーション**：タブナビゲーション（ホーム・ゴール・設定）

### Phase 2 以降の機能（後期実装）

1. **ガイド付き点検セッション**：5 ステップ質問をウィザード形式で実施
2. **アクションリスト**：Step 4 で決めた "小さな一歩" を自動 ToDo 化
3. **履歴 & ダッシュボード**：解消したボトルネック数・達成タスク数をグラフ表示
4. **プッシュ通知**：週次／月次のセッションをプッシュ通知で促進

> 📄 **詳細**: [点検セッション詳細フロー](./session_flow_details.md) を参照
...
| 画面             | ナビゲーション     | 主な内容                       |
| ---------------- | ------------------ | ------------------------------ |
| ホーム           | タブナビゲーション | 今日のゴール／アクションカード |
| 点検セッション   | スタック/モーダル  | 5 ステップウィザード           |
| ゴール管理       | タブナビゲーション | ゴール CRUD ＋ 並び替え        |
| ダッシュボード   | タブナビゲーション | グラフ表示・履歴タイムライン   |
| 設定             | タブナビゲーション | 通知設定・課金管理             |
| オンボーディング | 初回起動時         | アプリ紹介・使い方説明         |
...
- 無料 → 有料転換率目標：**4–6 %**
- Premium ARPU：**¥600 / 月**
- ブレイクイーブン：10,000 MAU × 5 % × 600 ≒ **¥300,000 / 月**
...
### Phase 1: ゴール設定機能（2 週間）- 最初のデプロイ

**目標**: ゴール設定機能のみを実装し、早期フィードバックを獲得

**Phase 1 機能範囲**:

- ✅ ユーザー認証（Supabase Auth）
- ✅ ゴールの作成・編集・削除・一覧表示
- ✅ 基本的なタブナビゲーション（ホーム・ゴール・設定）
- ✅ 基本 UI コンポーネント（Button, Input, Card）
- ✅ Expo デプロイ（EAS Build）

### Phase 2: 点検セッション機能（3 週間）

**目標**: 点検セッション機能とダッシュボード機能の実装

### Phase 3: App Store リリース（1 週間）
